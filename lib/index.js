...
function reduceReducers(reducers) {
    ...
    const reducerFuncs = Object.prototype.toString.call(reducers) === '[object Object]'
        ? Object.values(reducers)
        : reducers;
    return (previous, current) => reducerFuncs.reduce((p, r) => r(p, current), previous);
}
                    ...
                    if (this.config.injectAsyncLoading) {
                        ...
                        const newReducer = (state = rootState[curModel.namespace], action) => {
                            if (action.type !== `${curModel.namespace}/$$${actionCreatorName}Loading`)
                                return state;
                            return Object.assign(Object.assign({}, state), { [loadingKey]: action.payload[loadingKey] });
                        };
        ...
        const actionCreatorFeatures = {
            save: function (namespace, funcName, updateState, assignNamespace) {
                ...
                if (assignNamespace && typeof assignNamespace === 'string') {
                    ...
                    if (curModel.namespace === curNamespace) {
                        console.warn(`${WARNING_PREFIX} ${funcName} WARNING: namespace '${assignNamespace}' assigning in 'save' function is unnecessary`);
                    }
            ...
            pick: function (namespace, funcName, stateKey, assignNamespace) {
                ...
                if (assignNamespace && typeof assignNamespace === 'string') {
                    ...
                    if (curModel.namespace === curNamespace) {
                        console.warn(`${WARNING_PREFIX} ${funcName} WARNING: namespace '${assignNamespace}' assigning in 'pick' function is unnecessary`);
                    }
                    curNamespace = assignNamespace;
                }
